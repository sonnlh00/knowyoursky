...
public class GetWeatherForecast {
    ...
    @Inject
    public GetWeatherForecast(PrefsHelper prefsHelper) {
    ...
    public void getCachedCurrentForecast(Observer<CurrentForecast> observer){
        Observable<CurrentForecast> observable = Observable.create(new ObservableOnSubscribe<CurrentForecast>() {
            @Override
            public void subscribe(@NonNull ObservableEmitter<CurrentForecast> emitter) throws Throwable {
                emitter.onNext(processCurrentData(prefsHelper.getCurrentForecast()));
                emitter.onComplete();
            }
        })
                .subscribeOn(SchedulerProvider.io())
    ...
    public void getCachedHourlyForecast(Observer<List<HourlyForecast>> observer){
        Observable<List<HourlyForecast>> observable = Observable.create(new ObservableOnSubscribe<List<HourlyForecast>>() {
            @Override
            public void subscribe(@NonNull ObservableEmitter<List<HourlyForecast>> emitter) throws Throwable {
                emitter.onNext(processHourlyData(prefsHelper.getHourlyForecast()));
                emitter.onComplete();
            }
        })
                .subscribeOn(SchedulerProvider.io())
    ...
    private CurrentForecast doCurrentForecastApi(String requestURL){
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            prefsHelper.putCurrentForecast(res);
            forecast = processCurrentData(res);
    ...
    private List<HourlyForecast> doHourlyForecastApi(String requestURL){
        ...
        try (Response response = client.newCall(request).execute()) {
            if (!response.isSuccessful()) throw new IOException("Unexpected code " + response);
            String res = response.body().string();
            prefsHelper.putHourlyForecast(res);
            forecast = processHourlyData(res);
        } catch (IOException | JSONException e) {
            e.printStackTrace();
        }
        return forecast;
    ...
    private CurrentForecast processCurrentData(String weatherData) throws JSONException{
        ...
        if (resObject.has("cod") && resObject.getInt("cod") == 200) {
            ...
            des = (!des.equals("")) ? des.substring(0,1).toUpperCase() + des.substring(1) : "";
            ...
            dateFormatter.setTimeZone(
                    TimeZone.getTimeZone((timezone / 3600) > 0 ?
                            String.format(Locale.US,"GMT+%d", timezone/3600)
                            : String.format(Locale.US,"GMT%d", timezone / 3600))
            );
